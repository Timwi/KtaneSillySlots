...
public class AssetBundler
{
    ...
    protected static void BuildModBundle(bool useMSBuild)
    {
        ...
        if (ModConfigs.Instance == null 
            || ModConfigs.ID == ""
            || ModConfigs.OutputFolder == "")
        {
            Debug.LogError("You must configure your mod from the \"Keep Talking ModKit / Configure Mod\" menu first.");
            return;
        }

        Debug.LogFormat("Creating \"{0}\" AssetBundle...", ModConfigs.BundleName);

        AssetBundler bundler = new AssetBundler();

        bundler.assemblyName = ModConfigs.ID;
        bundler.outputFolder = ModConfigs.OutputFolder + "/" + bundler.assemblyName;
        bundler.bundleName = ModConfigs.BundleName;
    ...
    void CompileAssemblyWithEditor()
    {
        ...
        string unityExtensionsLocation = EditorApplication.applicationPath.Replace("Unity.exe", "Data/UnityExtensions/Unity/");

        managedReferences.Add(unityAssembliesLocation + "UnityEngine");
        managedReferences.Add(unityExtensionsLocation + "GUISystem/UnityEngine.UI");
...
#pragma warning restore 618
        ...
        string srcPath = Path.Combine(TEMP_BUILD_FOLDER, bundleName);
        string destPath = Path.Combine(outputFolder, "mod.bundle");
    ...
    {
        File.WriteAllText(outputFolder + "/modInfo.json", ModConfigs.Instance.Active.ToJson());
    }
    ...
    protected void WarnIfExampleAssetsAreIncluded()
    {
        ...
        {
            int numAssetsInBundle = AssetDatabase.FindAssets("b:" + bundleName).Length;
            int numExampleAssetsInBundle = AssetDatabase.FindAssets("b:" + bundleName, new string[] { examplesFolder }).Length;
    ...
    protected void WarnIfAssetsAreNotTagged()
    {
        ...
        foreach (var assetGUID in assetGUIDs)
        {
            ...
            if (!path.StartsWith("Assets/Examples") && IsIncludedAssetPath(path))
            {
                ...
                if (!importer.assetBundleName.Equals(bundleName))
                {
                    Debug.LogWarningFormat("Asset \"{0}\" is not tagged for {1} and will not be included in the AssetBundle!", path, bundleName);
                }
            }
        }
    ...
    {
        string[] assetsInBundle = AssetDatabase.FindAssets(string.Format("t:prefab,t:audioclip,t:scriptableobject,b:", bundleName));
        if (assetsInBundle.Length == 0)
        {
            throw new Exception(string.Format("No assets have been tagged for inclusion in the {0} AssetBundle.", bundleName));
        }
    }
